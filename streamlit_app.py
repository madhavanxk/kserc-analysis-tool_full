    ...
    if run_button:
        ...
        def validate_kseb_petition(pdf_path: str) -> tuple:
            ...
            try:
                ...
                # Level 0: Positive petition identity check on page 1
                # The KSEB petition's cover page always contains:
                #   - "APPLICANT" (role label in the filing table)
                #   - "Vydyuthi Bhavanam" (KSEB's registered address)
                # A KSERC order's page 1 contains neither — it opens with
                # "ORDER", the commission seal, and case reference numbers.
                # Strategy: REQUIRE at least one positive petition marker on page 1.
                petition_identity_markers = [
                    'vydyuthi bhavanam',
                    'vydyuthi bhavan',
                    'applicant',
                ...
                found_petition_identity = any(
                    kw in first_page_text for kw in petition_identity_markers
                )

                if not found_petition_identity:
                    return False, (
                        "This does not appear to be a KSEB truing-up petition. "
                        "The first page does not contain expected petition identifiers "
                        "(KSEB's name, address, or 'APPLICANT' label). "
                        "Please upload the petition filed BY KSEB Ltd — not a KSERC order."
                    ), {'pages': page_count}
